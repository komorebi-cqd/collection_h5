	...
	<view class="wyb-tabs-box" :style="boxStyleStr">
		<scroll-view 
		 ...
		 :style="{height: height + 'rpx'}">
			<view class="wyb-tabs-scroll-box" :style="{height: height + 'rpx', width: scroll ? 'fit-content': '100%'}">
				<view class="wyb-tabs-label" id="tabsBox">
					<view 
					 :id="'tab' + index"
					 class="wyb-tabs-label-item"
					 :class="{
						 'active': Number(currentUseForLeft) === index
					 }"
				...
				<view v-if="showSBlock" class="wyb-tabs-block" :style="{
					width: sBlockMode === 'fixed' ? sBlockWidth + 'rpx' : everytabsWidth[currentUseForLeft] + Number(sBlockType.paddingHori) * 2 + 'rpx',
		...
		computed: {
			...
			autoTabsItemStyle() {
				return function(index) {
					...
					return {
						marginLeft: this.scroll ? (index === 0 ? `${padpx}px` : `${padpx / 2}px`) : 0,
						marginRight: this.scroll ? (index === this.tabs.length - 1 ? `${padpx}px` : `${padpx / 2}px`) : 0,
						fontSize: `${uni.upx2px(Number(this.fontSize))}px`,
						width: this.scroll ? 'auto' : '100%',
						'justify-content': this.scroll ? 'flex-start' : 'center',
			...
			autoLeft() {
				...
				let result = 0
				result += (this.everytabsWidth[this.currentUseForLeft] + pad * 2) / 2 + pad  * this.currentUseForLeft + this.leftFix 
				for (let i = 1; i <= this.currentUseForLeft; i++) {
					result += this.everytabsWidth[this.currentUseForLeft - i]
				}
				if(isNaN(result)){ //如果出现NaN,则获取每个tab宽度
					this.initEverytabsWidth()
				}
				return `${result}px`
			},
			autoFixedLeft() {
		...
		methods: {
			...
			initEverytabsWidth() {
				...
				for(let i =0 ; i< this.tabs.length; i++){
					query.select('#tab'+i).boundingClientRect(data => {
						if(times<this.tabs.length){
							result.push(data.width)
							times++
						}
						console.log('执行'+times+ '次')
					}).exec();
					
					
				}
				this.everytabsWidth = result
				this.$forceUpdate()
			 ...
			 */
			async getTabsWidth(){
				return await this.getRect('.wyb-tabs-box')
			}
		}
	}
</script>
